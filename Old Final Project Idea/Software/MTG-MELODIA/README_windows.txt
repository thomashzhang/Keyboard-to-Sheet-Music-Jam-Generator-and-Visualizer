MELODIA - Melody Extraction Vamp plug-in 1.0 (windows)
======================================================
...
Conditions of Use
-----------------
The MELODIA 1.0 Vamp plug-in is offered free of charge for non-commercial use only. MELODIA is not free software, meaning you can not redistribute it nor modify it. If you are interested in using MELODIA for commercial purposes please contact: mtg-info@llista.upf.edu
...
Installation
------------
1. If not yet installed, please install the MS VS2010 Redistributable Package (see link above)
2. Copy all files in "MTG-MELODIA 1.0 (windows).zip" to:
...
2. Make sure the MELODIA plug-in is recognised by Sonic Annotator (requires adding the MELODIA dll location to your vamp path or launching the annotator from the dll directory). You can check the list of recognised plug-ins by running:
>sonic-annotator.exe -l
3. For a single file, run:
>sonic-annotator.exe -d vamp:mtg-melodia:melodia:melody AUDIOFILE_PATH -w csv
Or
>sonic-annotator.exe -d vamp:mtg-melodia:melodiaviz:OUTPUT_TYPE AUDIOFILE_PATH -w csv
...
>sonic-annotator.exe -d vamp:mtg-melodia:melodiaviz:saliencefunction C:\audio\song.wav -w csv
...
justin.salamon@gmail.com AND mtg-info@llista.upf.edu
